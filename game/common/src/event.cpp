...
EventDebugger::EventDebugger() {
    m_listener_id = COMMON_CONTEXT.m_event_system->AddListener<DebugEvent>(
        [&](EventListenerID, const DebugEvent& event) { m_triggered_count++; });
}

EventDebugger::~EventDebugger() {
    COMMON_CONTEXT.m_event_system->RemoveListener<DebugEvent>(m_listener_id);
}

void EventDebugger::SendDebugEvent(int value) {
    COMMON_CONTEXT.m_event_system->EnqueueEvent(DebugEvent{value});
}
