...
template <typename T>
std::vector<KeyFrame<T>>* tryGetKeyframes(AnimationTrackBase* track,
    ...
    if (type == AnimationTrackType::Linear) {
        auto* typed =
            dynamic_cast<AnimationTrack<T, AnimationTrackType::Linear>*>(track);
        return typed ? &typed->GetKeyframes() : nullptr;
    }
    auto* typed =
        dynamic_cast<AnimationTrack<T, AnimationTrackType::Discrete>*>(track);
    return typed ? &typed->GetKeyframes() : nullptr;
}

template <typename T>
void insertKeyframeByTime(std::vector<KeyFrame<T>>& keyframes, TimeType time,
                          const T& value) {
...
// Match a keyframe after reordering (sort); time compared with epsilon for
// double noise.
template <typename T>
...
std::unique_ptr<AnimationTrackBase> createTrackByType(AnimationTrackType type) {
    if (type == AnimationTrackType::Linear) {
        return std::make_unique<
            AnimationTrack<T, AnimationTrackType::Linear>>();
    }
    return std::make_unique<AnimationTrack<T, AnimationTrackType::Discrete>>();
}

template <typename T>
bool replaceTrackType(Animation& anim, bool is_bind_track,
                      const std::string& bind_name,
                      AnimationBindingPoint binding,
                      AnimationTrackType new_type,
...
void drawDashedLine(ImDrawList* draw, float x0, float x1, float y,
                    ImU32 color) {
...
void AnimationEditorContext::Init() {
    if (!instance) {
        instance = std::unique_ptr<AnimationEditorContext>(
            new AnimationEditorContext{});
    } else {
...
void AnimationEditorContext::update() {
    ...
    showInspectorPanel();
    showTimelinePanel();
    // After timeline scrub/seek, apply pose same frame before drawing the
    // preview.
    renderScenePreview();
...
void AnimationEditorContext::showInspectorPanel() {
    ...
    if (!m_current_animation) {
        ImGui::TextWrapped(
            "Open an animation file (File → Open Animation) to begin editing.");
        ImGui::End();
        return;
    }
    ...
    if (m_prefab) {
        if (auto filename = m_prefab.GetFilename(); filename) {
            ...
            ImGui::InputText("Preview prefab", text.data(), text.size() + 1,
    ...
    auto* player = (m_preview_entity == null_entity)
                       ? nullptr
                       : m_animation_player_manager->Get(m_preview_entity);
    if (!player) {
        ImGui::Separator();
        ImGui::TextWrapped(
            "Internal error: no AnimationPlayer on preview entity.");
        ImGui::End();
        return;
    }
    ...
    auto& sel = *m_selected_keyframe;
    std::string_view name = sel.m_is_bind_point_track
                                ? sel.m_bind_point_name.c_str()
                                : trackName(sel.m_binding);
    ImGui::Text("Track: %s", name.data());
    ImGui::Text("Mode: %s", sel.m_track_type == AnimationTrackType::Linear
                                ? "Linear"
                                : "Discrete");
    ...
    auto editFrame = [&](auto dummy) {
        ...
        auto* frames =
            tryGetKeyframes<value_type>(selected_track, sel.m_track_type);
        if (!frames) {
            return;
        }
        if (sel.m_keyframe_index < 0 ||
            sel.m_keyframe_index >= static_cast<int>(frames->size())) {
            return;
        }
        auto& frame = (*frames)[sel.m_keyframe_index];
        ImGui::DragScalar("Time", ImGuiDataType_Double, &frame.m_time, 0.01f,
                          nullptr, nullptr);
        if (frame.m_time < 0.0) {
            frame.m_time = 0.0;
        }
        // Do not sort every frame: update() runs Inspector before the timeline,
        // so sorting here invalidated keyframe drag indices.
    ...
    if (sel.m_is_bind_point_track ||
        sel.m_binding == AnimationBindingPoint::TransformPosition ||
...
void AnimationEditorContext::showTimelinePanel() {
    ...
    auto seekPreviewTo = [&](TimeType t) {
        ...
        const TimeType clamped_t = std::min<TimeType>(
            std::max<TimeType>(0.0, t), player->GetMaxTime());
        // Update() advances by (delta * rate). Here `clamped_t` is absolute
        // animation time, not clock delta — without forcing rate=1, scrubbing
        // scales with Rate and can overshoot GetMaxTime() in one step (e.g.
        // jump to end when moving toward the second keyframe).
    ...
    auto tryDrawTrackTyped = [&](AnimationTrackBase* track_base, auto fn) {
        ...
        if (dynamic_cast<AnimationTrack<Vec2, AnimationTrackType::Linear>*>(
                track_base) ||
            dynamic_cast<AnimationTrack<Vec2, AnimationTrackType::Discrete>*>(
                track_base)) {
            fn(Vec2{});
            return true;
        }
        if (dynamic_cast<AnimationTrack<Degrees, AnimationTrackType::Linear>*>(
                track_base) ||
            dynamic_cast<
                AnimationTrack<Degrees, AnimationTrackType::Discrete>*>(
                track_base)) {
            fn(Degrees{});
            return true;
        }
        if (dynamic_cast<
                AnimationTrack<Flags<Flip>, AnimationTrackType::Discrete>*>(
                track_base)) {
            fn(Flags<Flip>{});
            return true;
        }
    ...
    ImGui::SetCursorScreenPos(ImVec2(
        child_origin.x + m_track_name_panel_width - 2.0f, child_origin.y));
    ImGui::InvisibleButton("track_name_splitter",
                           ImVec2(4.0f, std::max(60.0f, child_height)));
    if (ImGui::IsItemActive()) {
        m_track_name_panel_width += ImGui::GetIO().MouseDelta.x;
        m_track_name_panel_width =
            Clamp(m_track_name_panel_width, 180.0f, 900.0f);
    }
    ImGui::GetWindowDrawList()->AddLine(
        ImVec2(child_origin.x + m_track_name_panel_width, child_origin.y),
        ImVec2(child_origin.x + m_track_name_panel_width,
               child_origin.y + std::max(60.0f, child_height)),
        IM_COL32(100, 100, 100, 180), 1.0f);
    ImGui::SetCursorScreenPos(child_origin);

    const float axis_h = 28.0f;
    const float line_min_x =
        child_origin.x + m_track_name_panel_width + kTrackPadding;
    const float line_max_x = line_min_x + timeline_pixels -
                             2.0f * kTrackPadding - kTrackTypeButtonWidth;
    const float scroll_x = ImGui::GetScrollX();
    auto timeToRow = [&](TimeType time) -> float {
        return line_min_x - scroll_x +
               static_cast<float>(time * m_timeline_zoom);
    };
    auto rowToTime = [&](float x) -> TimeType {
        return std::max<TimeType>(
            0.0, static_cast<TimeType>((x - (line_min_x - scroll_x)) /
                                       m_timeline_zoom));
    };

    ImGui::InvisibleButton(
        "timeline_axis_hit",
        ImVec2(m_track_name_panel_width + timeline_pixels, axis_h));
    ImDrawList* draw = ImGui::GetWindowDrawList();
    const ImVec2 axis_min = child_origin;
    const ImVec2 axis_max{
        child_origin.x + m_track_name_panel_width + timeline_pixels,
        child_origin.y + axis_h};
    const bool axis_hovered =
        ImGui::IsMouseHoveringRect(axis_min, axis_max, true);
    draw->AddRectFilled(axis_min, axis_max, IM_COL32(20, 20, 20, 180));
    draw->AddLine(ImVec2(line_min_x, axis_max.y - 2.0f),
                  ImVec2(line_max_x, axis_max.y - 2.0f),
                  IM_COL32(180, 180, 180, 255), 1.2f);
    ...
    if (px_per_sec >= 4000.0f)
        major_step_s = 0.001f;
    else if (px_per_sec >= 2500.0f)
        major_step_s = 0.002f;
    else if (px_per_sec >= 1200.0f)
        major_step_s = 0.005f;
    else if (px_per_sec >= 700.0f)
        major_step_s = 0.01f;
    else if (px_per_sec >= 350.0f)
        major_step_s = 0.02f;
    else if (px_per_sec >= 180.0f)
        major_step_s = 0.05f;
    else if (px_per_sec >= 90.0f)
        major_step_s = 0.1f;
    else if (px_per_sec >= 45.0f)
        major_step_s = 0.2f;
    else if (px_per_sec >= 25.0f)
        major_step_s = 0.5f;
    else if (px_per_sec >= 12.0f)
        major_step_s = 1.0f;
    else if (px_per_sec >= 6.0f)
        major_step_s = 2.0f;
    else
        major_step_s = 5.0f;
    ...
    for (float t = 0.0f; t < max_s; t += minor_step_s) {
        ...
        const bool major =
            major_mod < 0.0001f || (major_step_s - major_mod) < 0.0001f;
        const float tick_h = major ? 11.0f : 6.0f;
        draw->AddLine(ImVec2(x, axis_max.y - 2.0f),
                      ImVec2(x, axis_max.y - 2.0f - tick_h),
        ...
        if (major && (x - last_label_x) >= 70.0f) {
            ...
            draw->AddText(ImVec2(x + 2.0f, axis_min.y + 4.0f),
    ...
    if (player && player->HasAnimation()) {
        ...
        draw->AddLine(ImVec2(playhead_x, axis_min.y),
                      ImVec2(playhead_x, child_origin.y + child_height),
        ...
        if (axis_hovered && ImGui::IsMouseReleased(ImGuiMouseButton_Left) &&
            !drag_playhead) {
            seekPreviewTo(rowToTime(ImGui::GetIO().MousePos.x));
        }
    ...
    auto drawTrackRow = [&](const char* label, AnimationBindingPoint binding,
                            bool is_bind_track, const std::string& bind_name,
        ...
        const ImVec2 row_end{row_start.x + row_w,
                             row_start.y + kTrackRowHeight};
        ImDrawList* draw = ImGui::GetWindowDrawList();
        draw->AddRectFilled(row_start, row_end, IM_COL32(35, 35, 35, 80));

        const ImVec2 label_min = row_start;
        const ImVec2 label_max{row_start.x + m_track_name_panel_width,
                               row_end.y};
        draw->AddRectFilled(label_min, label_max, IM_COL32(25, 25, 25, 120));
        draw->PushClipRect(label_min, label_max, true);
        draw->AddText(ImVec2(label_min.x + 8.0f, label_min.y + 6.0f),
                      IM_COL32_WHITE, label);
        draw->PopClipRect();

        const float line_y = (row_start.y + row_end.y) * 0.5f;
        // Use the same time<->screen mapping as the ruler/playhead (line_min_x
        // + scroll), not row-local coords — tree indent + missing scroll_x made
        // scrubbing far too sensitive.
        ...
        if (track_base->GetType() == AnimationTrackType::Discrete) {
            drawDashedLine(draw, track_line_x0, track_line_x1, line_y,
                           IM_COL32(120, 120, 120, 255));
        } else {
            draw->AddLine(ImVec2(track_line_x0, line_y),
                          ImVec2(track_line_x1, line_y),
        ...
        auto drawAndHandleFrames = [&](auto dummy) {
            using value_type = decltype(dummy);
            auto* frames =
                tryGetKeyframes<value_type>(track_base, track_base->GetType());
            if (!frames) {
                return;
            }
            for (int i = 0; i < static_cast<int>(frames->size()); ++i) {
                ...
                const bool selected =
                    m_selected_keyframe &&
                    m_selected_keyframe->m_is_bind_point_track ==
                        is_bind_track &&
                    m_selected_keyframe->m_binding == binding &&
                    m_selected_keyframe->m_bind_point_name == bind_name &&
                    m_selected_keyframe->m_track_type ==
                        track_base->GetType() &&
                    m_selected_keyframe->m_keyframe_index == i;
                const ImU32 color = selected ? IM_COL32(255, 210, 90, 255)
                                             : IM_COL32(240, 240, 240, 255);
                draw->AddCircleFilled(ImVec2(x, line_y), kTrackKeyRadius,
                                      color);

                const ImVec2 p = ImGui::GetIO().MousePos;
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
                    p.x >= x - kTrackKeyRadius && p.x <= x + kTrackKeyRadius &&
                    p.y >= line_y - kTrackKeyRadius &&
                    p.y <= line_y + kTrackKeyRadius) {
                    m_selected_keyframe =
                        SelectedKeyframe{is_bind_track, binding, bind_name,
                                         track_base->GetType(), i};
            ...
            if (drag_state.active &&
                ...
                drag_state.bind_name == bind_name &&
                drag_state.type == track_base->GetType() &&
                drag_state.key_index >= 0 &&
                drag_state.key_index < static_cast<int>(frames->size())) {
                auto& frame = (*frames)[drag_state.key_index];
                frame.m_time =
                    std::max<float>(0.0f, rowToTime(ImGui::GetIO().MousePos.x));
                seekPreviewTo(frame.m_time);
            }

            if (drag_state.active &&
                ImGui::IsMouseReleased(ImGuiMouseButton_Left)) {
                if (drag_state.is_bind == is_bind_track &&
                    drag_state.binding == binding &&
                    drag_state.bind_name == bind_name &&
                    drag_state.type == track_base->GetType() &&
                    drag_state.key_index >= 0 &&
                    drag_state.key_index < static_cast<int>(frames->size())) {
                    const KeyFrame<value_type> moved =
                        (*frames)[drag_state.key_index];
                    std::sort(frames->begin(), frames->end(),
                              [](const auto& a, const auto& b) {
                                  return a.m_time < b.m_time;
                              });
                    ...
                    bool found = false;
                    for (int idx = 0; idx < static_cast<int>(frames->size());
                         ++idx) {
                        if (keyframesMatchLogicalIdentity((*frames)[idx],
                                                          moved)) {
                            new_index = idx;
                            found = true;
                            break;
            ...
            const bool label_hovered =
                ImGui::IsMouseHoveringRect(label_min, label_max, true);
            if (label_hovered &&
            ...
            if (ImGui::BeginPopup("track_row_menu")) {
                const TimeType insert_time =
                    rowToTime(ImGui::GetIO().MousePos.x);
                if (ImGui::MenuItem("Insert Keyframe Here")) {
                    insertKeyframeByTime(*frames, insert_time,
                                         defaultValueFromFrames(*frames));
                }
                ImGui::EndPopup();
            }
            ...
            const bool can_to_linear =
                supportLinear(binding) &&
                track_base->GetType() == AnimationTrackType::Discrete;
            const bool can_to_discrete =
                supportDiscrete(binding) &&
                track_base->GetType() == AnimationTrackType::Linear;
            const bool can_toggle = can_to_linear || can_to_discrete;
            ImGui::SetCursorScreenPos(
                ImVec2(row_start.x + row_w - kTrackTypeButtonWidth - 6.0f,
                       row_start.y + 4.0f));
            if (!can_toggle) {
            ...
            const char* preview =
                track_base->GetType() == AnimationTrackType::Linear
                    ? "Linear"
                    : "Discrete";
            if (ImGui::BeginCombo("##track_type", preview,
                                  ImGuiComboFlags_NoArrowButton) &&
                can_toggle) {
                if (can_to_linear && ImGui::Selectable("Linear")) {
                    if (replaceTrackType<value_type>(
                            *m_current_animation, is_bind_track, bind_name,
                            binding, AnimationTrackType::Linear, *frames)) {
                        clearSelectedKeyframe();
                ...
                if (can_to_discrete && ImGui::Selectable("Discrete")) {
                    if (replaceTrackType<value_type>(
                            *m_current_animation, is_bind_track, bind_name,
                            binding, AnimationTrackType::Discrete, *frames)) {
                        clearSelectedKeyframe();
                        refreshPlayerAnimationBinding();
                    }
                }
        ...
        if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
            ImGui::IsItemHovered() && !clicked_keyframe) {
            const TimeType clicked_time =
                std::max<float>(0.0f, rowToTime(ImGui::GetIO().MousePos.x));
        ...
        if (ImGui::IsItemHovered() &&
            ImGui::IsMouseDown(ImGuiMouseButton_Left) && !clicked_keyframe &&
            !drag_state.active && !drag_state.keyframe_awaiting_drag) {
            const TimeType drag_time =
                std::max<float>(0.0f, rowToTime(ImGui::GetIO().MousePos.x));
    ...
    if (ImGui::BeginPopup("create_track_popup")) {
        static AnimationBindingPoint create_binding =
        ...
        if (ImGui::Button("Create")) {
            if (create_binding == AnimationBindingPoint::BindPoint) {
                ...
                m_current_animation->GetTracks()[create_binding] =
                    std::make_unique<AnimationTrack<
                        Flags<Flip>, AnimationTrackType::Discrete>>();
            } else if (create_binding == AnimationBindingPoint::SpriteImage) {
                m_current_animation->GetTracks()[create_binding] =
                    std::make_unique<AnimationTrack<
                        ImageHandle, AnimationTrackType::Discrete>>();
            } else {
                m_current_animation->GetTracks()[create_binding] =
                    createTrackByType<Vec2>(create_type);
            }
    ...
    if (ImGui::CollapsingHeader("Property Tracks",
    ...
    if (ImGui::CollapsingHeader("Bind Point Tracks",
                                ImGuiTreeNodeFlags_DefaultOpen)) {
        for (auto& [name, track] : m_current_animation->GetBindPointTracks()) {
            std::string label = std::string("BindPoint.") + name;
            drawTrackRow(label.c_str(), AnimationBindingPoint::BindPoint, true,
                         name, track.get());
        }
    }
    ...
    if (pending_delete.valid) {
        if (pending_delete.is_bind) {
            m_current_animation->GetBindPointTracks().erase(
                pending_delete.bind_name);
        } else {
...
void AnimationEditorContext::loadAnimation(Path filename) {
    auto relative = ToProjectRelative(filename);
    auto handle =
        m_assets_manager->GetManager<Animation>().Load(relative, true);
    if (!handle) {
...
void AnimationEditorContext::saveAnimation() {
    ...
    SaveAsset(m_current_animation.GetUUID(), *m_current_animation,
