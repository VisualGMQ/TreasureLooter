...
bool Action::IsPressed(SDL_JoystickID id) const {
    ...
    auto gamepad = CLIENT_CONTEXT.m_gamepad_manager->Find(id);
    if (gamepad) {
...
bool Action::IsPress(SDL_JoystickID id) const {
    return IsPressed(id) || IsPressing(id);
}
...
float Axis::Value(SDL_JoystickID gamepad_id) const {
    ...
    if (auto gamepad = CLIENT_CONTEXT.m_gamepad_manager->Find(gamepad_id)) {
...
void InputManager::Initialize(InputConfigHandle config, ClientContext& context) {
    SetConfig(context, config);
}

void InputManager::SetConfig(ClientContext& context, InputConfigHandle config) {
...
void InputManager::loadAxisConfig(ClientContext& context,
...
void InputManager::loadActionConfig(ClientContext& context,
