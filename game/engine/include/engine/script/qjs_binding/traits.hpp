#pragma once
#include "engine/script/qjs_binding/util/type_list.hpp"
#include "engine/script/qjs_binding/class_id.hpp"
#include "engine/script/qjs_binding/common.hpp"
#include "engine/script/qjs_binding/value_wrapper.hpp"
#include "quickjs.h"
...
template <typename T>
struct FnParamConverter<T&, std::enable_if_t<IsPureClass<T>>> {
    static T& Convert(JSContext* ctx, JSValue value) {
        ...
        TL_ASSERT(false, "[quickjs]: invalid argument convert");
    }
};
...
template <typename T>
struct FnParamConverter<const T*, std::enable_if_t<IsPureClass<T>>> {
...
struct JSConstructorTraits {
    static JSValue Fn(JSContext* ctx, JSValueConst new_target, int argc,
                      JSValueConst* argv) {
        TL_ASSERT(argc == sizeof...(Args), "wrong number of arguments");
...
private:
    ...
    static Class* callConstructor(JSContext* ctx, JSValueConst* argv,
        ...
        return new Class{
            FnParamConverter<list_element_t<args_list, Indices>>::Convert(
                ctx, argv[Indices])...};
    }
};
...
private:
    ...
    static return_type callFn(JSContext* ctx, JSValue self, JSValue* argv,
        ...
        TL_ASSERT(id != 0, "call function on unregistered class");
    ...
    static return_type doCallFn(JSContext* ctx, T&& self, JSValue* argv,
        ...
        if constexpr (sizeof...(Indices) == 0) {
            ...
            return std::invoke(
                ...
                FnParamConverter<list_element_t<args_list, Indices>>::
...
struct JSFnTraits {
    ...
    static JSValue Fn(JSContext* ctx, JSValueConst self, int argc,
                      JSValueConst* argv) {
        TL_ASSERT(argc == list_size_v<args_list>, "wrong number of parameters");
...
struct JSMemberVariableTraits {
    using traits = variable_pointer_traits<F>;
    using clazz = typename traits::clazz;
