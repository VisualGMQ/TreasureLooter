...
public:
    explicit QJSClassIDManager(JSRuntime* runtime)
        : m_runtime{runtime} {
    }
...
public:
    ...
    template <typename T, typename = std::enable_if_t<IsPureClass<T>>>
...
namespace std {
template <>
struct hash<QJSClassIDFamily> {
    size_t operator()(
        const QJSClassIDFamily& p) const noexcept {
