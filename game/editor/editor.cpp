...
struct AssetSyncHelper {
    template <typename T>
    void operator()(AssetLoadResult<T>& payload) {
        EDITOR_CONTEXT.m_assets_manager->GetManager<T>().Reload(payload.m_uuid);
    }

    void operator()(AssetLoadResult<InputConfig>& payload) {
        auto handle =
            EDITOR_CONTEXT.m_assets_manager->GetManager<InputConfig>().Replace(
                payload.m_uuid, payload.m_payload);
        EDITOR_CONTEXT.m_input_manager->SetConfig(EDITOR_CONTEXT, handle);
    }

    void operator()(AssetLoadResult<Prefab>& payload) {
        EDITOR_CONTEXT.m_assets_manager->GetManager<Prefab>().Replace(
            payload.m_uuid, payload.m_payload);
...
void AddEntityToScene(Entity entity, AssetLoadResult<Prefab>& instance) {
    ...
    auto relationship = EDITOR_CONTEXT.m_relationship_manager->Get(root_entity);
...
AssetTypes LoadAssetFromPath<Prefab>(const Path& filename) {
    ...
    if (auto level = EDITOR_CONTEXT.m_level_manager->GetCurrentLevel()) {
        level->Instantiate(
            EDITOR_CONTEXT.m_assets_manager->GetManager<Prefab>().Find(
                result.m_uuid));
    }
